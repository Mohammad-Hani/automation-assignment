resource "azurerm_public_ip" "loadbalancer-pubip" {
  count               = 2
  name                = "${var.loadbalancer-pubip}-lbpip-7${format("%1d", count.index + 1)}"
  location            = var.location
  resource_group_name = var.resource_group
  domain_name_label   = "lbdns-7${format("%1d", count.index + 1)}"
...
resource "azurerm_network_interface_backend_address_pool_association" "loadbalancer_pool_association" {
  ...
  ip_configuration_name   = "lvm-3726-ipconfig-${each.value + 1}"
