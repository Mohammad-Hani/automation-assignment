...
variable "loadbalancer-pubip" {
    default = "3726-loadbalancer-pubip"
}

variable "loadbalancer" {
    type = map(string) 
    default = {
        name    = "3726-loadbalancer"
        frontend_ip_configuration = "pubip-3552"
    }
}

variable "backend_address_pool" {
    default = "3726-backendaddresspool"
}

variable "loadbalancer_pool_association" {
    default = "3726-loadbalancer-pool-association"
}

variable "loadbalancer_rule" {
    type = map(string) 
    default = {
        name = "3726-loadbalancer-rule"
        protocol = "Tcp"
        frontend_port = 3389
        backend_port = 3389
